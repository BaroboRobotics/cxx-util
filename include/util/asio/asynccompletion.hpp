...
struct AsyncCompletion {
    ...
    explicit AsyncCompletion (typename std::remove_reference<CompletionToken>::type& token)
        : handler(static_cast<IfTokenIsHandler<HandlerType&, CompletionToken&&>>(token))
        , result(handler) {}
